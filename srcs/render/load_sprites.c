...
void	load_static_entites_frames(t_game *g)
{
	...
	g->collectibles_sp.frames = malloc(NUM_COIN_FRAMES * sizeof(t_img));
	if (!g->collectibles_sp.frames)
		message(g, "Failed allocation on collectibles frames\n");
	g->walls_sp.nframes = NUM_WALLS;
	g->floor_sp.nframes = NUM_FLOOR_FRAMES;
	g->exit_sp.nframes = NUM_EXIT_FRAMES;
	g->collectibles_sp.nframes = NUM_COIN_FRAMES;
